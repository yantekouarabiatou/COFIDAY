...
@section('content')
    ...
    <div class="col-md-6">
        <div class="form-group">
            ...
            @if($manager)
                <input type="hidden" name="superieur_hierarchique_id" value="{{ $manager->id }}">
                <div class="form-control-plaintext">
                    <i class="fas fa-user-tie me-2"></i>
                    {{ $manager->prenom }} {{ $manager->nom }} — {{ $manager->email }}
                </div>
                <small class="form-text text-muted">Votre supérieur hiérarchique est défini dans votre profil.</small>
            @else
                <div class="alert alert-danger">
                    <i class="fas fa-exclamation-triangle"></i>
                    Aucun supérieur hiérarchique n’est défini pour votre compte. Veuillez contacter l’administrateur.
                </div>
                <input type="hidden" name="superieur_hierarchique_id" value="">
            @endif
